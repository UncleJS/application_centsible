  ...
  return (
    <div className="flex flex-col gap-8">
      <PageHeader
        eyebrow="Recurring"
        title="Subscriptions"
        description="Track recurring charges, normalized monthly cost, and the next renewals due."
        action={
          <Button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <StatCard
            label="Monthly Cost"
            value={ratesPending ? "—" : formatCurrency(totalMonthlyCost, userCurrency)}
            hint={ratesPending ? "Loading exchange rates…" : "Normalized across all billing cycles"}
            icon={CreditCard}
          />
          <StatCard
            label="Active Subscriptions"
            value={String(activeCount)}
            hint="Active recurring services"
            icon={RefreshCw}
          />
          <StatCard
            label="Next Renewal"
            value={nextRenewal ? nextRenewal.name : "—"}
            hint={nextRenewal && nextRenewalDays !== null ? `${nextRenewalDays < 0 ? "Overdue" : nextRenewalDays === 0 ? "Today" : `In ${nextRenewalDays} day${nextRenewalDays === 1 ? "" : "s"}`} · ${formatDate(nextRenewal.nextRenewalDate)}` : "No renewal scheduled"}
            icon={CalendarClock}
            accentClassName={nextRenewalDays !== null ? getRenewalColor(nextRenewalDays) : undefined}
          />
        </div>
      )}
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Add your recurring services to track normalized monthly cost and upcoming renewals."
          action={
            <Button
              onClick={handleCreate}
              className="bg-emerald-600 text-white hover:bg-emerald-500"
