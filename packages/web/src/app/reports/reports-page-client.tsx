  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Review monthly income, expenses, category usage, and longer-term trends."
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="rounded-2xl bg-zinc-900 p-1">
        ...
        <TabsContent value="overview" className="space-y-6">
          {summaryLoading ? (
            ...
            <EmptyState
              ...
              description={`No data for ${getMonthName(selectedMonth)} ${selectedYear}. Add transactions and budgets to generate insights.`}
            />
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <StatCard
                  ...
                  hint={`${getMonthName(selectedMonth)} ${selectedYear}`}
                  icon={TrendingUp}
                ...
                <StatCard
                  ...
                  hint={`${summary.byCategory.length} category ${summary.byCategory.length === 1 ? "entry" : "entries"}`}
                  icon={parseFloat(summary.netAmount) >= 0 ? TrendingUp : TrendingDown}
                  accentClassName={netColorClass()}
                />
              </div>
